...
- 📥 **`/pull` 补拉机制** — 用户回复后发送 `/pull`，按微信长度上限分块拉取未送达消息，默认单块上限为 `5200` 字符
...
> 🧪 **关于 `/pull` 分块默认值**：当前默认 `PULL_CHUNK_LIMIT=5200`。该值来自 2026-04-20 的现网实测：`3500`、`5000`、`5250`、`5312`、`5375`、`5437`、`5468` 均可直接发送，`5484`、`5500`、`6000`、`8000` 会进入缓存队列。默认值因此保守下调到 `5200`，同时保留环境变量覆盖能力。
